...
def test_blender_scene_properties():
  scene = core.Scene(
      frame_start=2,
      frame_end=3,
      frame_rate=5,
...
def test_blender_camera_on_init():
  cam = core.PerspectiveCamera(position=(1, 2, 3), quaternion=(0, 1, 0, 0), focal_length=3,
                               sensor_width=4)
  renderer = blender.Blender(core.Scene(camera=cam))
...
def test_blender_camera_assign_after_init():
  scene = core.Scene()
  renderer = blender.Blender(scene)
...
def test_blender_adaptive_sampling_default():
...
def test_blender_set_adaptive_sampling():
  renderer = blender.Blender(core.Scene())
...
def test_blender_init_adaptive_sampling():
  renderer = blender.Blender(core.Scene(), adaptive_sampling=False)
  assert renderer.adaptive_sampling is False
  assert renderer.blender_scene.cycles.use_adaptive_sampling is False
...
def test_blender_set_samples_per_pixel():
  renderer = blender.Blender(core.Scene())
...
def test_blender_samples_per_pixel_init():
  renderer = blender.Blender(core.Scene(), samples_per_pixel=256)
